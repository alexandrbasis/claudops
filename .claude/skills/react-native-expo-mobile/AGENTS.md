...
> **New Architecture (2026):** As of Expo SDK 55, the New Architecture (Fabric + TurboModules) is always enabled and cannot be disabled. Reanimated 4, native bottom tabs, and many performance features require it. All rules in this document assume New Architecture is active.
...
3. [Animation](#3-animation) — **HIGH**
   ...
   - 3.4 [Use CSS Transitions for Simple State-Driven Animations](#34-use-css-transitions-for-simple-state-driven-animations-reanimated-4)
...
### 3.4 Use CSS Transitions for Simple State-Driven Animations (Reanimated 4+)

**Impact: HIGH (declarative animations, less boilerplate)**

Reanimated 4 introduces CSS-like transition props that run on the UI thread. For
simple state-driven animations (show/hide, color changes, size nudges), prefer
`transitionProperty`/`transitionDuration` over manual `useAnimatedStyle` +
`withTiming`. This reduces boilerplate and makes animations declarative.

> **Note:** Reanimated 4 requires the New Architecture (Fabric). SDK 55+ enables
> New Architecture by default.

**Before (Reanimated 3): manual animated style**
...
function FadeBox({ visible, children }: { visible: boolean; children: React.ReactNode }) {
  const animatedStyle = useAnimatedStyle(() => ({
    opacity: withTiming(visible ? 1 : 0),
    transform: [{ scale: withTiming(visible ? 1 : 0.95) }],
  }))
...
**After (Reanimated 4): CSS transitions**
...
- **CSS transitions**: State-driven settle effects (show/hide, color/opacity,
  size/position nudges, tab indicators)
- **Keyframes** (`animationName`): Loops and staged effects (pulse, bounce,
  shimmer)
- **Shared values + `useAnimatedStyle`**: Gesture-driven animations, complex
  interpolations, scroll-linked animations

Reference: [Reanimated CSS Animations](https://docs.swmansion.com/react-native-reanimated/docs/css/css-animations)
